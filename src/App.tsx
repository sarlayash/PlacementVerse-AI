...
export default function App() {
  ...
  const handleStartJourney = (
    ...
    const updated: LearnerProfile = {
      ...profile,
      name: name.trim(),
      institute: institute.trim() || profile?.institute || 'National Institute of Technology',
      department: department.trim() || profile?.department || 'Computer Science & Engineering',
      badgesEarned: Array.isArray(profile?.badgesEarned) ? profile.badgesEarned : ['streak-fire-1'],
      completedTopicIds: Array.isArray(profile?.completedTopicIds) ? profile.completedTopicIds : [],
      unlockedTopicIds: Array.isArray(profile?.unlockedTopicIds) ? profile.unlockedTopicIds : ['mod1-topic-1'],
      dailyMissions: Array.isArray(profile?.dailyMissions) && profile.dailyMissions.length > 0 ? profile.dailyMissions : [
      ...
      realWorldSubmissions: profile?.realWorldSubmissions || {},
